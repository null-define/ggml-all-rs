...
use std::sync::Arc;
use ggml_all_sys_2::whisper_token;
...
pub struct FullParams<'a, 'b> {
    pub(crate) fp: ggml_all_sys_2::whisper_full_params,
    phantom_lang: PhantomData<&'a str>,
    phantom_tokens: PhantomData<&'b [c_int]>,
    grammar: Option<Vec<ggml_all_sys_2::whisper_grammar_element>>,
...
impl<'a, 'b> FullParams<'a, 'b> {
    ...
    pub fn new(sampling_strategy: SamplingStrategy) -> FullParams<'a, 'b> {
        let mut fp = unsafe {
            ggml_all_sys_2::whisper_full_default_params(match sampling_strategy {
                SamplingStrategy::Greedy { .. } => {
                    ggml_all_sys_2::WHISPER_SAMPLING_GREEDY
                }
                SamplingStrategy::BeamSearch { .. } => {
                    ggml_all_sys_2::WHISPER_SAMPLING_BEAM_SEARCH
                }
            } as _)
        };
        ...
        O: Into<Option<F>>,
    {
        ...
        use ggml_all_sys_2::{whisper_context, whisper_state};
            ...
            F: FnMut(SegmentCallbackData) + 'static,
        {
            unsafe {
                ...
                let n_segments = ggml_all_sys_2::whisper_full_n_segments_from_state(state);
                let s0 = n_segments - n_new;
                //let user_data = user_data as *mut Box<dyn FnMut(SegmentCallbackData)>;

                for i in s0..n_segments {
                    let text = ggml_all_sys_2::whisper_full_get_segment_text_from_state(state, i);
                    let text = CStr::from_ptr(text);

                    let t0 = ggml_all_sys_2::whisper_full_get_segment_t0_from_state(state, i);
                    let t1 = ggml_all_sys_2::whisper_full_get_segment_t1_from_state(state, i);
