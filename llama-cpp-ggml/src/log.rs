...
fn meta_for_level(
    level: ggml_all_sys_2::ggml_log_level,
) -> (&'static Metadata<'static>, &'static OverridableFields) {
    match level {
        ggml_all_sys_2::GGML_LOG_LEVEL_DEBUG => (&DEBUG_META, &DEBUG_FIELDS),
        ggml_all_sys_2::GGML_LOG_LEVEL_INFO => (&INFO_META, &INFO_FIELDS),
        ggml_all_sys_2::GGML_LOG_LEVEL_WARN => (&WARN_META, &WARN_FIELDS),
        ggml_all_sys_2::GGML_LOG_LEVEL_ERROR => (&ERROR_META, &ERROR_FIELDS),
        _ => {
...
pub(super) struct State {
    ...
    module: Module,
    buffered: std::sync::Mutex<Option<(ggml_all_sys_2::ggml_log_level, String)>>,
...
impl State {
    ...
    fn generate_log(target: Module, level: ggml_all_sys_2::ggml_log_level, text: &str) {
    ...
    pub(super) fn cont_buffered_log(&self, text: &str) {
        ...
        if let Some((previous_log_level, mut buffer)) = lock.take() {
            ...
            let level = self
                ...
                as ggml_all_sys_2::ggml_log_level;
            tracing::warn!(
    ...
    pub(super) fn buffer_non_cont(&self, level: ggml_all_sys_2::ggml_log_level, text: &str) {
        debug_assert!(!text.ends_with('\n'));
        debug_assert_ne!(level, ggml_all_sys_2::GGML_LOG_LEVEL_CONT);
    ...
    pub(super) fn emit_non_cont_line(&self, level: ggml_all_sys_2::ggml_log_level, text: &str) {
        debug_assert!(text.ends_with('\n'));
        debug_assert_ne!(level, ggml_all_sys_2::GGML_LOG_LEVEL_CONT);
        ...
        match level {
            ggml_all_sys_2::GGML_LOG_LEVEL_NONE => {
                // TODO: Support logging this to stdout directly via options?
                tracing::info!(no_log_level = true, text);
            }
            ggml_all_sys_2::GGML_LOG_LEVEL_DEBUG
            | ggml_all_sys_2::GGML_LOG_LEVEL_INFO
            | ggml_all_sys_2::GGML_LOG_LEVEL_WARN
            | ggml_all_sys_2::GGML_LOG_LEVEL_ERROR => Self::generate_log(self.module, level, text),
            ggml_all_sys_2::GGML_LOG_LEVEL_CONT => unreachable!(),
            _ => {
                tracing::warn!(
                    level = level,
                    text = text,
