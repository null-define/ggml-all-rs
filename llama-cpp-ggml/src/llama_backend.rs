...
impl LlamaBackend {
    ...
    pub fn init() -> crate::Result<LlamaBackend> {
        Self::mark_init()?;
        unsafe { ggml_all_sys_2::llama_backend_init() }
        Ok(LlamaBackend {})
    }
    ...
    pub fn init_numa(strategy: NumaStrategy) -> crate::Result<LlamaBackend> {
        Self::mark_init()?;
        unsafe {
            ggml_all_sys_2::llama_numa_init(ggml_all_sys_2::ggml_numa_strategy::from(strategy));
        }
        Ok(LlamaBackend {})
    }
    ...
    pub fn void_logs(&mut self) {
        ...
        unsafe {
            ggml_all_sys_2::llama_log_set(Some(void_log), std::ptr::null_mut());
        }
    }
}
...
pub struct InvalidNumaStrategy(
    ...
    pub ggml_all_sys_2::ggml_numa_strategy,
);

impl TryFrom<ggml_all_sys_2::ggml_numa_strategy> for NumaStrategy {
    type Error = InvalidNumaStrategy;

    fn try_from(value: ggml_all_sys_2::ggml_numa_strategy) -> Result<Self, Self::Error> {
        match value {
            ggml_all_sys_2::GGML_NUMA_STRATEGY_DISABLED => Ok(Self::DISABLED),
            ggml_all_sys_2::GGML_NUMA_STRATEGY_DISTRIBUTE => Ok(Self::DISTRIBUTE),
            ggml_all_sys_2::GGML_NUMA_STRATEGY_ISOLATE => Ok(Self::ISOLATE),
            ggml_all_sys_2::GGML_NUMA_STRATEGY_NUMACTL => Ok(Self::NUMACTL),
            ggml_all_sys_2::GGML_NUMA_STRATEGY_MIRROR => Ok(Self::MIRROR),
            ggml_all_sys_2::GGML_NUMA_STRATEGY_COUNT => Ok(Self::COUNT),
            value => Err(InvalidNumaStrategy(value)),
        }
    }
}

impl From<NumaStrategy> for ggml_all_sys_2::ggml_numa_strategy {
    fn from(value: NumaStrategy) -> Self {
        match value {
            NumaStrategy::DISABLED => ggml_all_sys_2::GGML_NUMA_STRATEGY_DISABLED,
            NumaStrategy::DISTRIBUTE => ggml_all_sys_2::GGML_NUMA_STRATEGY_DISTRIBUTE,
            NumaStrategy::ISOLATE => ggml_all_sys_2::GGML_NUMA_STRATEGY_ISOLATE,
            NumaStrategy::NUMACTL => ggml_all_sys_2::GGML_NUMA_STRATEGY_NUMACTL,
            NumaStrategy::MIRROR => ggml_all_sys_2::GGML_NUMA_STRATEGY_MIRROR,
            NumaStrategy::COUNT => ggml_all_sys_2::GGML_NUMA_STRATEGY_COUNT,
        }
    }
}
...
impl Drop for LlamaBackend {
    fn drop(&mut self) {
        ...
        unsafe { ggml_all_sys_2::llama_backend_free() }
    }
}
...
mod tests {
    ...
    fn numa_from_and_to() {
        ...
        for numa in &numas {
            let from = ggml_all_sys_2::ggml_numa_strategy::from(*numa);
