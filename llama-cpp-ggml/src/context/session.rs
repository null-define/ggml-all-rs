...
impl LlamaContext {
    ...
    pub fn save_session_file(
        ...
        if unsafe {
            ggml_all_sys_2::llama_save_session_file(
                self.context.lock().unwrap().as_ptr(),
                cstr.as_ptr(),
                tokens.as_ptr().cast::<ggml_all_sys_2::llama_token>(),
                tokens.len(),
            )
        } {
            Ok(())
    ...
    pub fn load_session_file(
        ...
        let tokens_out = tokens.as_mut_ptr().cast::<ggml_all_sys_2::llama_token>();

        let load_session_success = unsafe {
            ggml_all_sys_2::llama_load_session_file(
                self.context.lock().unwrap().as_ptr(),
                cstr.as_ptr(),
                tokens_out,
                max_tokens,
    ...
    pub fn get_state_size(&self) -> usize {
        unsafe { ggml_all_sys_2::llama_get_state_size(self.context.lock().unwrap().as_ptr()) }
    }
    ...
    pub unsafe fn copy_state_data(&self, dest: *mut u8) -> usize {
        unsafe { ggml_all_sys_2::llama_copy_state_data(self.context.lock().unwrap().as_ptr(), dest) }
    }
    ...
    pub unsafe fn set_state_data(&mut self, src: &[u8]) -> usize {
        unsafe { ggml_all_sys_2::llama_set_state_data(self.context.lock().unwrap().as_ptr(), src.as_ptr()) }
    }

    pub fn get_seq_state_data(&mut self, dst: &mut [u8], size: usize, seq_id: i32) -> usize {
        unsafe {
            ggml_all_sys_2::llama_state_seq_get_data(
                self.context.lock().unwrap().as_ptr(),
                dst.as_mut_ptr(),
                size,
                seq_id,
    ...
    pub fn set_seq_state_data(&mut self, src: &[u8], size: usize, seq_id: i32) -> usize {
        unsafe {
            ggml_all_sys_2::llama_state_seq_set_data(
                self.context.lock().unwrap().as_ptr(),
                src.as_ptr(),
                size,
                seq_id,
