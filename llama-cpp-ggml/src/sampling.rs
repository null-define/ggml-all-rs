...
pub struct LlamaSampler {
    pub(crate) sampler: Mutex<*mut ggml_all_sys_2::llama_sampler>,
}
...
impl LlamaSampler {
    ...
    pub fn sample(&mut self, ctx: &LlamaContext, idx: i32) -> LlamaToken {
        let token = unsafe {
            ggml_all_sys_2::llama_sampler_sample(*self.sampler.lock().unwrap(), ctx.context.lock().unwrap().as_ptr(), idx)
        };
    ...
    pub fn accept(&mut self, token: LlamaToken) {
        unsafe { ggml_all_sys_2::llama_sampler_accept(*self.sampler.lock().unwrap(), token.0) }
    }
    ...
    pub fn accept_many(&mut self, tokens: impl IntoIterator<Item = impl Borrow<LlamaToken>>) {
        for token in tokens {
            unsafe { ggml_all_sys_2::llama_sampler_accept(*self.sampler.lock().unwrap(), token.borrow().0) }
        }
    }
    ...
    pub fn reset(&mut self) {
        unsafe {
            ggml_all_sys_2::llama_sampler_reset(*self.sampler.lock().unwrap());
        }
    }
    ...
    pub fn get_seed(&self) -> u32 {
        unsafe { ggml_all_sys_2::llama_sampler_get_seed(*self.sampler.lock().unwrap()) }
    }    
    ...
    pub fn chain(samplers: impl IntoIterator<Item = Self>, no_perf: bool) -> Self {
        unsafe {
            let chain = ggml_all_sys_2::llama_sampler_chain_init(
                ggml_all_sys_2::llama_sampler_chain_params { no_perf },
            );

            for sampler in samplers {
                ggml_all_sys_2::llama_sampler_chain_add(chain, *sampler.sampler.lock().unwrap());
    ...
    pub fn temp(t: f32) -> Self {
        let sampler = unsafe { ggml_all_sys_2::llama_sampler_init_temp(t) };
    ...
    pub fn temp_ext(t: f32, delta: f32, exponent: f32) -> Self {
        let sampler = unsafe { ggml_all_sys_2::llama_sampler_init_temp_ext(t, delta, exponent) };
        Self { sampler: sampler.into() }
    }
    ...
    pub fn top_k(k: i32) -> Self {
        let sampler = unsafe { ggml_all_sys_2::llama_sampler_init_top_k(k) };
        Self { sampler: sampler.into() }
    }
    ...
    pub fn top_n_sigma(n: f32) -> Self {
        let sampler = unsafe { ggml_all_sys_2::llama_sampler_init_top_n_sigma(n) };
        Self { sampler: sampler.into() }
    }
    ...
    pub fn typical(p: f32, min_keep: usize) -> Self {
        let sampler = unsafe { ggml_all_sys_2::llama_sampler_init_typical(p, min_keep) };
        Self { sampler: sampler.into() }
    }
    ...
    pub fn top_p(p: f32, min_keep: usize) -> Self {
        let sampler = unsafe { ggml_all_sys_2::llama_sampler_init_top_p(p, min_keep) };
        Self { sampler: sampler.into() }
    }
    ...
    pub fn min_p(p: f32, min_keep: usize) -> Self {
        let sampler = unsafe { ggml_all_sys_2::llama_sampler_init_min_p(p, min_keep) };
        Self { sampler: sampler.into() }
    }
    ...
    pub fn xtc(p: f32, t: f32, min_keep: usize, seed: u32) -> Self {
        let sampler = unsafe { ggml_all_sys_2::llama_sampler_init_xtc(p, t, min_keep, seed) };
        Self { sampler: sampler.into() }
    }
    ...
    pub fn grammar(model: &LlamaModel, grammar_str: &str, grammar_root: &str) -> Self {
        ...
        let sampler = unsafe {
            ggml_all_sys_2::llama_sampler_init_grammar(
                model.vocab_ptr(),
                grammar_str.as_ptr(),
                grammar_root.as_ptr(),
            )
    ...
    pub fn grammar_lazy(
        ...
        let sampler = unsafe {
            ggml_all_sys_2::llama_sampler_init_grammar_lazy(
                model.vocab_ptr(),
                grammar_str.as_ptr(),
                grammar_root.as_ptr(),
    ...
    pub fn dry(
        ...
        let sampler = unsafe {
            ggml_all_sys_2::llama_sampler_init_dry(
                model.vocab_ptr(),
                model
                    .n_ctx_train()
                    .try_into()
    ...
    pub fn penalties(
        ...
        let sampler = unsafe {
            ggml_all_sys_2::llama_sampler_init_penalties(
                penalty_last_n,
                penalty_repeat,
                penalty_freq,
                penalty_present,
    ...
    pub fn mirostat(n_vocab: i32, seed: u32, tau: f32, eta: f32, m: i32) -> Self {
        let sampler =
            unsafe { ggml_all_sys_2::llama_sampler_init_mirostat(n_vocab, seed, tau, eta, m) };
        Self { sampler: sampler.into() }
    }
    ...
    pub fn mirostat_v2(seed: u32, tau: f32, eta: f32) -> Self {
        let sampler = unsafe { ggml_all_sys_2::llama_sampler_init_mirostat_v2(seed, tau, eta) };
        Self { sampler: sampler.into() }
    }
    ...
    pub fn dist(seed: u32) -> Self {
        let sampler = unsafe { ggml_all_sys_2::llama_sampler_init_dist(seed) };
        Self { sampler: sampler.into() }
    }
    ...
    #[must_use]
    pub fn greedy() -> Self {
        let sampler = unsafe { ggml_all_sys_2::llama_sampler_init_greedy() };
        Self { sampler: sampler.into() }
    }
    ...
    pub fn logit_bias(n_vocab: i32, biases: &[LlamaLogitBias]) -> Self {
        ...
        let data = biases.as_ptr().cast::<ggml_all_sys_2::llama_logit_bias>();
        
        let sampler = unsafe {
            ggml_all_sys_2::llama_sampler_init_logit_bias(
                n_vocab,
                biases.len() as i32,
                data,
            )
...
impl Drop for LlamaSampler {
    fn drop(&mut self) {
        unsafe {
            ggml_all_sys_2::llama_sampler_free(*self.sampler.lock().unwrap());
        }
    }
}
