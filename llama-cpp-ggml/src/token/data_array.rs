...
impl LlamaTokenDataArray {
    ...
    pub(crate) unsafe fn modify_as_c_llama_token_data_array<T>(
        &mut self,
        modify: impl FnOnce(&mut ggml_all_sys_2::llama_token_data_array) -> T,
    ) -> T {
        let size = self.data.len();
        let data = self
            .data
            .as_mut_ptr()
            .cast::<ggml_all_sys_2::llama_token_data>();

        let mut c_llama_token_data_array = ggml_all_sys_2::llama_token_data_array {
            data,
            size,
    ...
    pub fn apply_sampler(&mut self, sampler: &LlamaSampler) {
        unsafe {
            self.modify_as_c_llama_token_data_array(|c_llama_token_data_array| {
                ggml_all_sys_2::llama_sampler_apply(*sampler.sampler.lock().unwrap(), c_llama_token_data_array);
            });
        }
    }
